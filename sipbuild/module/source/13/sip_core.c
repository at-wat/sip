...
static void *sip_api_type_get_extension_data(const sipTypeDef *td,
        const char *build_system_extension_name);
static int sip_api_type_set_extension_data(sipTypeDef *td,
        const char *build_system_extension_name, void *data);
...
static const sipAPIDef sip_api = {
    ...
    sip_api_type_get_extension_data,
    NULL,
    NULL,
    NULL,
    /*
    ...
    sip_api_type_set_extension_data,
    NULL,
    NULL,
    NULL,
};
...
/*
 * Any entry in the linked list of extension datum.
 */
typedef struct _extension_data_link {
    const char *build_system_extension_name;    /* The build system name. */
    void *data;                                 /* The extension data. */
    struct _extension_data_link *next;          /* The next in the list. */
} extension_data_link;


/*
 * Return any extension data for a type generated by a build system extension.
 */
static void *sip_api_type_get_extension_data(const sipTypeDef *td,
        const char *build_system_extension_name)
{
    extension_data_link *link;

    for (link = (extension_data_link *)(td->td_plugin_data); link != NULL; link = link->next)
        if (strcmp(link->build_system_extension_name, build_system_extension_name) == 0)
            return link->data;
...
/*
 * Set the extension data for a type generated by a build system extension.
 */
static int sip_api_type_set_extension_data(sipTypeDef *td,
        const char *build_system_extension_name, void *data)
{
    /*
     * Note that a build system shouldn't call this more than once for the same
     * type, but we don't check.
     */

    extension_data_link *link;

    if ((link = sip_api_malloc(sizeof (extension_data_link))) == NULL)
        return -1;

    link->build_system_extension_name = build_system_extension_name;
    link->data = data;
    link->next = td->td_plugin_data;

    td->td_plugin_data = link;
